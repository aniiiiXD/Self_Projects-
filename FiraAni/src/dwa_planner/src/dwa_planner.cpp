...
// #include "dwa_planner/msg/CustomMessage.h"
      ...
      scan_not_subscribe_count_(0)
{
    ...
    //local_nh_.param<double>("HZ", hz_, {12});
    local_nh_.param<double>("/dm_node/loop_rate", hz_, {12});
    ...
    local_nh_.param<int>("SUBSCRIBE_COUNT_TH", subscribe_count_th_, {0});
    ...
    // Primary published data : Command Velocity :  Format of custom message
    //velocity_pub_ = nh_.advertise<geometry_msgs::Twist>("/cmd_vel", 1);  // Change the message type here and publish the data
    trajectory_pub_ = nh_.advertise<geometry_msgs::PoseArray>("/best_trajectory", 1); // New publisher for publishing the entire trajectory in the required data type
    // To Controls


    // These are local topics only for the visualisation of the planner : Not necessary for the final planner
    ...
    obs_list_pub = local_nh_.advertise<geometry_msgs::PoseArray>("/obs_list",1);
    ...
    goal_sub_ = nh_.subscribe("/local_goal_dm", 1, &DWAPlanner::goal_callback, this); // From DecisionMaking
    local_map_sub_ = nh_.subscribe("/occupancy_grid", 1, &DWAPlanner::local_map_callback, this); // From ComputerVision
    odom_sub_ = nh_.subscribe("/final_speed", 1, &DWAPlanner::odom_callback, this); // From Mechatronics
    angular_sub_ = nh_.subscribe("/filtered_yaw", 1, &DWAPlanner::angular_callback, this); // From Mechatronics - IMU (To be changed later)
...
// Get position of the local goal for the planner
...
// Get the occupancy grid for the local environment of the planner
...
// Get the current velocity and yawrate of the vehicle
//void DWAPlanner::odom_callback(const nav_msgs::OdometryConstPtr &msg)
void DWAPlanner::odom_callback(const std_msgs::Float64::ConstPtr &msg)
{
    current_cmd_vel_ = msg->data;
    odom_not_subscribe_count_ = 0;
    odom_updated_ = true;
}

void DWAPlanner::angular_callback(const std_msgs::Float64::ConstPtr &msg)
{
    current_ang_vel_ = msg->data;
    ang_not_subscribe_count_ = 0;
    ang_updated_ = true;
}
...
DWAPlanner::dwa_planning(const Eigen::Vector3d &goal, std::vector<std::pair<std::vector<State>, bool>> &trajectories)
{
    ...
    cout<<available_traj_count<<endl;
    ROS_INFO(" ");
...
void DWAPlanner::process(void)
{
    ...
    while (ros::ok())
    {
        ...
        std::pair<geometry_msgs::Twist, std::vector<State>> return_data;
        geometry_msgs::Twist cmd_vel;
        std::vector<State> best_traj;
        // if (can_move())
        return_data = calc_cmd_vel();
        cmd_vel = return_data.first;
        best_traj = return_data.second;

        geometry_msgs::PoseArray message; // Holds the trajectory information to be published for controls

        for (const auto&state : best_traj){
            geometry_msgs::Pose pose;
            pose.position.x = state.x_ + 0.0; // orginally just state.x_
            pose.position.y = state.y_ + 0.0; // orginally just state.y_
            pose.position.z = state.velocity_;
            pose.orientation.w = state.yawrate_;

            message.poses.push_back(pose);
        }


        //velocity_pub_.publish(cmd_vel);  // Published the command velocity -- Need to change this to the Entire State Path
        trajectory_pub_.publish(message); // New publisher message contains the x,y,v,w arrays of the best trajectory
        finish_flag_pub_.publish(has_finished_);
        obs_list_pub.publish(obs_list_);
        ...
        string str = to_string(current_cmd_vel_);
        ROS_INFO_STREAM(str);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed = end - start;

        ROS_INFO_STREAM("Time taken to execute the code: " << elapsed.count() << " seconds");
...
bool DWAPlanner::can_move(void)
{
    ...
    if (subscribe_count_th_ < local_map_not_subscribe_count_)
        ...
        ROS_INFO_STREAM("Local_map_not_subscribe_COUNT_TH: " << local_map_not_subscribe_count_);
...
std::pair<geometry_msgs::Twist, std::vector<DWAPlanner::State> > DWAPlanner::calc_cmd_vel(void)
{
    geometry_msgs::Twist cmd_vel;
    std::pair<std::vector<DWAPlanner::State>, bool> best_traj;
    std::vector<std::pair<std::vector<DWAPlanner::State>, bool> > trajectories;
    ...
    // Here return the Best Trajectory along with the command velocity
    std::pair<geometry_msgs::Twist, std::vector<DWAPlanner::State> > return_data;
    return_data.first = cmd_vel;
    return_data.second = best_traj.first;
...
DWAPlanner::Window DWAPlanner::calc_dynamic_window(void)
{
    ...
    window.min_velocity_ = std::max((current_cmd_vel_ - max_deceleration_ * dt_), min_velocity_);
    window.max_velocity_ = std::min((current_cmd_vel_ + max_acceleration_ * dt_), target_velocity_);
    window.min_yawrate_ = std::max((current_ang_vel_ - max_d_yawrate_ * dt_), -max_yawrate_);
    window.max_yawrate_ = std::min((current_ang_vel_ + max_d_yawrate_ * dt_), max_yawrate_);
    return window;
}
...
void DWAPlanner::raycast(const nav_msgs::OccupancyGrid &map)
{
    int counter = 0;
    obs_list_.poses.clear();
    const double max_search_dist = hypot(map.info.origin.position.x + 800, map.info.origin.position.y + 800);
    //const double max_search_dist = hypot(map.info.origin.position.x + 400, map.info.origin.position.y + 400);
    ...
    const double b, const ros::Publisher &pub)
{
    ...
    for (int count = 0; count < trajectories.size(); count++)
    {
        ...
        v_trajectory.header.frame_id = "camera_color_optical_frame";
