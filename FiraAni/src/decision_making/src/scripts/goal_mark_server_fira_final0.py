...
class DM_Node:
    ...
    def __init__(self):
        ...
        rospy.Subscriber(self.midpoint,Point , self.midpoint_callback )
    ...
    def find_nearest_lane(mid_x, mid_y, left_lane_x, left_lane_y, right_lane_x, right_lane_y):
    
        left_distance = ((mid_x - left_lane_x) ** 2 + (mid_y - left_lane_y) ** 2) ** 0.5
        right_distance = ((mid_x - right_lane_x) ** 2 + (mid_y - right_lane_y) ** 2) ** 0.5
    ...
    def midpoint_callback(self, msg):
        ...
        if x != 0 or y != 0:
            ...
            self.obstacle_lane_bool = self.find_nearest_lane(x, y, self.left_lane_x, self.left_lane_y, self.right_lane_x, self.right_lane_y)
        else:
            self.obstacle_detected = False
    ...
    def publishLocalGoal(self, x, y, yaw):
        ...
        self.local_goal_vis.pose.position.x = x * 0.00140625 + 80 * 0.0125    
        self.local_goal_vis.pose.position.y = y * 0.0010625 + 0.2 + 80 * 0.0125
    ...
    def run_algo(self):
        ...
        if not self.in_left_lane:
            ...
            if self.obstacle_detected:
                if not self.obstacle_lane_bool :
                ...
                while(time.time()-start_time <= 1.1 and self.delay <= 1 ):
                    rospy.loginfo("Obstacle detected! Keep Left")
                    max_len = np.max([len(self.ly), len(self.my), len(self.ry)])
                    if max_len == 0 :
                        return
