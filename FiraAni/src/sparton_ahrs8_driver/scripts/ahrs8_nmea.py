...
# Constants for serial communication commands in bytes
CMD_RESET_OUTPUT = b"\x13"
CMD_HDM = b"$xxHDM\r\n"
CMD_PRINTMASK = b"printmask 0 set drop\r\n"
CMD_PRINTMODULUS = b"printmodulus 0 set drop\r\n"
CMD_PRINTTRIGGER = b"printtrigger 0 set drop\r\n"
CMD_RESUME_OUTPUT = b"\x11"
CMD_GET_G = b"$PSPA,G\r\n"
CMD_GET_QUAT = b"$PSPA,QUAT\r\n"
CMD_GET_A = b"$PSPA,A\r\n"

# Verify the checksum obtained in the NMEA message.
def verify_checksum(response):
    # strip leading $
    message_and_checksum = response.strip(b"$")
    # split into message text and trailing checksum as a 2-tuple
    message_and_checksum = message_and_checksum.split(b"*")
    ...
    for character in message:
        calculated_checksum = calculated_checksum ^ character

    # Get the 2 digit hex representation of the result.
    hexstring = format(calculated_checksum, "02X").encode()
...
def populate_G(string, message):
    ...
    Gz_string = split_string[3].split(b"*")[0]

    # Parse the angular velocity values.
    Gx_float = millidegrees_to_radians(float(Gx_string.split(b"=")[1]))
    Gy_float = millidegrees_to_radians(float(Gy_string.split(b"=")[1]))
    Gz_float = millidegrees_to_radians(float(Gz_string.split(b"=")[1]))
...
def populate_QUAT(string, message):
    ...
    # Split checksum off of last output string.
    z_string = split_string[4].split(b"*")[0]

    # Parse the quaternion values.
    w_float = float(w_string.split(b"=")[1])
    x_float = float(x_string.split(b"=")[1])
    y_float = float(y_string.split(b"=")[1])
    z_float = float(z_string.split(b"=")[1])
...
def populate_A(string, message):
    ...
    # Split checksum off of last output string.
    Az_string = split_string[3].split(b"*")[0]

    # Parse the linear acceleration values.
    Ax_float = millig_to_meter(float(Ax_string.split(b"=")[1]))
    Ay_float = millig_to_meter(float(Ay_string.split(b"=")[1]))
    Az_float = millig_to_meter(float(Az_string.split(b"=")[1]))
...
if __name__ == '__main__':
    ...
    try:
        ...
        compass_serial.write(CMD_RESET_OUTPUT)
        compass_serial.write(CMD_HDM)
        compass_serial.write(CMD_PRINTMASK)
        compass_serial.write(CMD_PRINTMODULUS)
        compass_serial.write(CMD_PRINTTRIGGER)

        rospy.sleep(0.1)
        # Resume output allowed. Ctrl-Q
        compass_serial.write(CMD_RESUME_OUTPUT)
        rospy.sleep(0.1)
        # Flush all buffers.
    ...
    while not rospy.is_shutdown():
        ...
        compass_serial.write(CMD_GET_G)
        ...
        compass_serial.write(CMD_GET_QUAT)
        response = compass_serial.readline()
        if verify_checksum(response):
        ...
        compass_serial.write(CMD_GET_A)
        response = compass_serial.readline()
        if verify_checksum(response):
