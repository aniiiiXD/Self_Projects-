        ...
        while (ros::ok()) {
            ...
            if(!turn_flag_ ){
                ...
                if(ros::Time::now().toSec()-init_time>=required_time){
                    double next_hardcode_action=set_hardcode_status(next_hardcode_action);
                    // turn_flag_=false;
                    // next_status=0;
                    // init_time=-1.0;
...
private:
    ...
    void stopline_cb(const std_msgs::Bool::ConstPtr& msg) {
        ...
        ROS_INFO("                         \
                                            \
                                             STOOOOP");
        velocity_boost_=0;
        double next_hardcode_action = set_hardcode_status(2.0);
        }
    }
    ...
    void turn_rad_cb(const std_msgs::Float64::ConstPtr& msg) {
        ...
        double next_hardcode_action = set_hardcode_status(msg->data);
    }
