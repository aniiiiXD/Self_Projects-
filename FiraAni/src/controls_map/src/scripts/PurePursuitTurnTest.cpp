...
public:
	...
	nh_.getParam("/controls_node/MEAN_OFFSET", mean_offset);
    ...
    void control_loop() {
        ...
        while (ros::ok()) {
            ...
            if(!turn_flag_ and (ros::Time::now().toSec()-last_time>1 or last_time==-1)){
                ...
                double required_time=abs(M_PI*turn_radius_/turn_speed_);
                if(init_time==-1){
